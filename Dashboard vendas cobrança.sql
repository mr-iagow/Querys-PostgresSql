...
AND c.id IN (
	...
	AND DATE(a.conclusion_date) BETWEEN cast(date_trunc('month', current_date-INTERVAL '5 month') as date) AND DATE(curdate())
	)
...
(caa.activation_date BETWEEN cast(date_trunc('month', current_date-INTERVAL '5 month') as date) AND DATE(curdate())
AND ppg.people_group_id = 20
)
