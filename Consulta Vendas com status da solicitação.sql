SELECT DISTINCT ON (cst.contract_id)
    ...
    (select ppp.name from people AS ppp where ppp.id = C.seller_1_id) AS vendedor,
      CASE 
		WHEN ppg.people_group_id IS NULL THEN NULL
		ELSE (SELECT pg.title FROM people_groups AS pg WHERE pg.id = ppg.people_group_id) 
		END AS equipe,
    LAST_VALUE(r.description) OVER (PARTITION BY cst.contract_id ORDER BY r.created ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS ultimo_relato,
...
WHERE
    ...
    AND DATE(a.created) BETWEEN '2023-11-01' AND '2023-11-29'
