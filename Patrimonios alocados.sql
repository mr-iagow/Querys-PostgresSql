...
inv.client_name,
(SELECT cp.description FROM companies_places AS cp where cp.id = c.company_place_id) AS empresa_cliente,
c.company_place_id,
...
INNER JOIN people AS p ON p.id = inv.client_id
INNER JOIN contracts AS c ON c.client_id = p.id 


WHERE inv.financial_operation_id = 8 AND date(inv.created) BETWEEN '2023-02-01' AND '2023-02-10'